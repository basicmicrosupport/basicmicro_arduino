...
class Basicmicro : public Stream
{
	...
	enum {  
			...
			CANOPENWRITELOCALDICT = 190,	//MCP only
			CANOPENREADLOCALDICT = 191,		//MCP only
...
public:
	...
	bool CANOpenWriteLocalDict(uint8_t address, uint16_t index, uint8_t subindex, uint32_t value, uint8_t size, uint32_t &result);
	bool CANOpenReadLocalDict(uint8_t address, uint16_t index, uint8_t subindex, uint32_t &value, uint8_t &size, uint8_t &type, uint32_t &result);
