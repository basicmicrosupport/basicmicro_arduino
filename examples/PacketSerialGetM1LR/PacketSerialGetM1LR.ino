/**
 * Basicmicro Library Example: GETM1LR (130) - MCP Only
 *
 * Demonstrates reading the configured motor inductance (L) and resistance (R)
 * parameters for Motor 1 on MCP series controllers.
 *
...
void setup() {
  ...
  Serial.println("Basicmicro GETM1LR Example (MCP Only)");
  ...
  Serial.println("Attempting to read Motor 1 L/R values...");
}
...
void loop() {
  ...
  // Attempt to read the Motor 1 L and R values
  // This command is typically only supported by MCP series controllers.
  // The function returns true on success, false on failure.
  // The values are stored in read_L and read_R.
  bool success = controller.GetM1LR(MOTOR_ADDRESS, read_L, read_R);

  if (success) {
    Serial.println("GETM1LR command successful.");
    Serial.println("Motor 1 L/R Parameters:");
    ...
    Serial.println("GETM1LR command failed.");
