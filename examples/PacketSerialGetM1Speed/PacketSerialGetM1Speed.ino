/**
 * Basicmicro Library Example: GETM1SPEED (18)
 *
 * Demonstrates reading the current speed value for Motor 1 from the
 * Basicmicro motor controller. This value represents the actual speed
 * of Motor 1 in encoder counts per second, typically used for velocity control feedback.
 *
...
void setup() {
  ...
  Serial.println("Basicmicro GETM1SPEED Example");
  ...
  Serial.println("Attempting to read Motor 1 speed...");
}
...
void loop() {
  ...
  // Attempt to read the speed value for Motor 1
  ...
  speedValue = controller.ReadSpeedM1(MOTOR_ADDRESS, &status, &valid);

  if (valid) {
    Serial.println("GETM1SPEED command successful.");
    ...
    */
    ...
    Serial.println("GETM1SPEED command failed.");
