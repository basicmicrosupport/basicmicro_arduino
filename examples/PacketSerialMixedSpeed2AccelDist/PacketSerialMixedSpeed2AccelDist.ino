/**
 * Basicmicro Library Example: MIXEDSPEED2ACCELDIST (51)
 *
 * Demonstrates commanding both Motor 1 and Motor 2 to move specific distances
 * with independent acceleration rates and at specified top speeds. This provides
 * fine-grained control over trapezoidal velocity profiles for each motor.
...
// Define example acceleration, speed, and distance for both motors independently
...
void setup() {
  ...
  Serial.println("Basicmicro MIXEDSPEED2ACCELDIST Example");
  ...
  Serial.print("Attempting to command M1 Accel/Speed/Dist: "); Serial.print(motor1_accel); Serial.print("/"); Serial.print(motor1_speed); Serial.print("/"); Serial.print(motor1_distance);
  Serial.print(", M2 Accel/Speed/Dist: "); Serial.print(motor2_accel); Serial.print("/"); Serial.print(motor2_speed); Serial.print("/"); Serial.print(motor2_distance);
  Serial.println(" (Immediate, Forward, Absolute)");
}

void loop() {
  // Attempt to send the SpeedAccelDistanceM1M2_2 command for both motors
  // The function expects uint32_t for accel, speed, and distance for each motor.
  bool success = controller.SpeedAccelDistanceM1M2_2(MOTOR_ADDRESS,
                                                      motor1_accel, motor1_speed, motor1_distance,
                                                      motor2_accel, motor2_speed, motor2_distance,
                                                      control_flag);

  if (success) {
    Serial.println("MIXEDSPEED2ACCELDIST command successful. Motors should start moving.");
    // In a real application, you would then monitor encoder positions or status
    // to know when the movement is complete.
  } else {
    Serial.println("MIXEDSPEED2ACCELDIST command failed.");
  ...
  if (millis() - lastCommandTime > 8000) { // Wait 8 seconds before sending the next command
      ...
      if (movingForward) {
          ...
          motor1_accel = 400; motor1_speed = 800; motor1_distance = 4000;
          motor2_accel = 500; motor2_speed = 1000; motor2_distance = 5000;
          ...
          Serial.print("Commanding M1/M2: Backward, Relative. M1 "); Serial.print(motor1_accel); Serial.print("/"); Serial.print(motor1_speed); Serial.print("/"); Serial.print(motor1_distance);
          Serial.print(", M2 "); Serial.print(motor2_accel); Serial.print("/"); Serial.print(motor2_speed); Serial.print("/"); Serial.print(motor2_distance);
          Serial.println();
          movingForward = false;
      } else {
          // Command forward relative movement with original parameters
          motor1_accel = 500; motor1_speed = 1000; motor1_distance = 5000;
          motor2_accel = 600; motor2_speed = 1200; motor2_distance = 6000;

           control_flag = (0 << 0) | (1 << 1) | (0 << 2) | // M1: Forward, Relative, Accel unlimited
                          (0 << 4) | (1 << 5) | (0 << 6) | // M2: Forward, Relative, Accel unlimited
                          (0 << 3);             // Immediate execution

          Serial.print("Commanding M1/M2: Forward, Relative. M1 "); Serial.print(motor1_accel); Serial.print("/"); Serial.print(motor1_speed); Serial.print("/"); Serial.print(motor1_distance);
          Serial.print(", M2 "); Serial.print(motor2_accel); Serial.print("/"); Serial.print(motor2_speed); Serial.print("/"); Serial.print(motor2_distance);
          Serial.println();
          movingForward = true;
      }
      // Send the command again with the new parameters
      controller.SpeedAccelDistanceM1M2_2(MOTOR_ADDRESS,
                                         motor1_accel, motor1_speed, motor1_distance,
                                         motor2_accel, motor2_speed, motor2_distance,
                                         control_flag);
      Serial.println("MIXEDSPEED2ACCELDIST command sent again.");
  }
