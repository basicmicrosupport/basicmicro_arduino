/**
 * Basicmicro Library Example: SETM1LR (128) - MCP Only
 *
 * Demonstrates setting the motor inductance (L) and resistance (R) parameters
 * for Motor 1 on MCP series controllers. These values are used for advanced
...
// Define example Motor 1 Inductance (L) and Resistance (R) values (floating point)
...
float motor1_inductance_H = 0.00025; // Example: 250 uH
float motor1_resistance_Ohm = 0.15;  // Example: 0.15 Ohm
...
void setup() {
  ...
  Serial.println("Basicmicro SETM1LR Example (MCP Only)");
  ...
  Serial.print("Attempting to set Motor 1 L: "); Serial.print(motor1_inductance_H, 6); // Print with more precision
  Serial.print(" H, R: "); Serial.print(motor1_resistance_Ohm, 4); // Print with more precision
  Serial.println(" Ohm");
}

void loop() {
  // Attempt to set the Motor 1 L and R values
  // This command is typically only supported by MCP series controllers.
  // The function returns true on success, false on failure.
  bool success = controller.SetM1LR(MOTOR_ADDRESS, motor1_inductance_H, motor1_resistance_Ohm);

  if (success) {
    Serial.println("SETM1LR command successful. Values set in RAM.");
    ...
    // Optionally, read back the setting to verify (using GETM1LR)
    /*
    float read_L, read_R;
    if (controller.GetM1LR(MOTOR_ADDRESS, read_L, read_R)) {
        Serial.println("Verified Motor 1 L/R:");
        Serial.print("  L: "); Serial.print(read_L, 6); Serial.println(" H");
        Serial.print("  R: "); Serial.print(read_R, 4); Serial.println(" Ohm");
    } else {
        Serial.println("Failed to read Motor 1 L/R for verification.");
    }
    */
    ...
    */
    ...
    Serial.println("SETM1LR command failed.");
