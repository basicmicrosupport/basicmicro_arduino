/**
 * Basicmicro Library Example: SETM1MAXCURRENT (133)
 *
 * Demonstrates setting the maximum allowable current limits for Motor 1.
...
// Define example maximum current limits for Motor 1 in 10s of milliamps (uint32_t)
// !!! These values are examples. You MUST use values appropriate for your motor. !!!
uint32_t m1_max_current_mA = 2000; // Example: 20 Amps Driving Current Limit
uint32_t m1_min_current_mA = 1000; // Example: 10 Amps Regenerative Braking Current Limit
...
void setup() {
  ...
  Serial.println("Basicmicro SETM1MAXCURRENT Example");
  ...
  Serial.print("Attempting to set Motor 1 Max Current (Driving/Regen) to: ");
  Serial.print(m1_max_current_mA); Serial.print(" mA / ");
  Serial.print(m1_min_current_mA); Serial.println(" mA");
}

void loop() {
  // Attempt to set the maximum current limits for Motor 1
  // The function expects uint32_t values for both max and min limits.
  bool success = controller.SetM1MaxCurrent(MOTOR_ADDRESS, m1_max_current_mA, m1_min_current_mA);

  if (success) {
    Serial.println("SETM1MAXCURRENT command successful. Limits set in RAM.");
    Serial.println("Remember to use WRITENVM (94) to save permanently (causes reset).");
    // Optionally, read back the setting to verify (using ReadM1MaxCurrent)
    /*
    uint32_t read_max, read_min;
    if (controller.ReadM1MaxCurrent(MOTOR_ADDRESS, read_max, read_min)) {
        Serial.println("Verified Motor 1 Max Current (Driving/Regen):");
        Serial.print("  Max: "); Serial.print(read_max); Serial.print(" mA, ");
        Serial.print("Min: "); Serial.print(read_min); Serial.println(" mA");
    } else {
        Serial.println("Failed to read Motor 1 Max Current for verification.");
    }
    */
    ...
    */
    ...
    Serial.println("SETM1MAXCURRENT command failed.");
