/**
 * Basicmicro Library Example: GETM1ENC (16)
 *
 * Demonstrates reading the encoder count value for Motor 1 from the
 * Basicmicro motor controller. This value represents the current position
 * of Motor 1 based on encoder feedback.
 *
...
void setup() {
  ...
  Serial.println("Basicmicro GETM1ENC Example");
  ...
  Serial.println("Attempting to read Motor 1 encoder...");
}
...
void loop() {
  ...
  // Attempt to read the encoder value for Motor 1
  ...
  encoderValue = controller.ReadEncM1(MOTOR_ADDRESS, &status, &valid);

  if (valid) {
    Serial.println("GETM1ENC command successful.");
    Serial.print("Motor 1 Encoder Count: ");
    Serial.println((int32_t)encoderValue); // Cast to signed int32_t for printing
    Serial.print("Motor 1 Encoder Status: ");
    ...
    */
    ...
    Serial.println("GETM1ENC command failed.");
    Serial.println("Check wiring, power, address, baud rate, and controller configuration (Is Motor 1 encoder enabled?).");
  }
