/**
 * Basicmicro Library Example: SETAUTO2 (106)
 *
 * Demonstrates setting the configuration value for "Auto Mode 2" on the
 * Basicmicro motor controller. The meaning and effect of this value are
 * highly dependent on the specific controller model and how "Auto Mode 2"
...
// Define the desired value for Auto Mode 2 (32-bit)
// !!! Replace this with a value derived from your controller's manual !!!
uint32_t auto2_value = 789012; // Example placeholder value
...
void setup() {
  ...
  Serial.println("Basicmicro SETAUTO2 Example");
  ...
  Serial.print("Attempting to set Auto Mode 2 value to: ");
  Serial.println(auto2_value);
  Serial.println("NOTE: The meaning of this value is specific to your controller model.");
}

void loop() {
  // Attempt to set the Auto Mode 2 value
  // The function returns true on success, false on failure.
  bool success = controller.SetAuto2(MOTOR_ADDRESS, auto2_value);

  if (success) {
    Serial.println("SETAUTO2 command successful. Value set in RAM.");
    ...
    if (controller.GetAutos(MOTOR_ADDRESS, read_auto1, read_auto2)) {
        ...
        Serial.print("  Auto Mode 1: "); Serial.println(read_auto1); // Will show current Auto1 value
        Serial.print("  Auto Mode 2: "); Serial.println(read_auto2); // Should match what we set
    } else {
    ...
    */
    ...
    Serial.println("SETAUTO2 command failed.");
