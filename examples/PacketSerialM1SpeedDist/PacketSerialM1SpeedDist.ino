/**
 * Basicmicro Library Example: M1SPEEDDIST (41)
 *
 * Demonstrates commanding Motor 1 to move a specific distance at a specified speed.
...
// Define example speed and distance for Motor 1
uint32_t motor1_speed = 400;     // Speed in counts/sec
uint32_t motor1_distance = 2000; // Distance in encoder counts
...
void setup() {
  ...
  Serial.println("Basicmicro M1SPEEDDIST Example");
...
void loop() {
  // Attempt to send the SpeedDistance command for Motor 1
  // The function expects uint32_t for speed and distance.
  bool success = controller.SpeedDistanceM1(MOTOR_ADDRESS, motor1_speed, motor1_distance, motor1_flag);

  if (success) {
    Serial.println("M1SPEEDDIST command successful. Motor should start moving.");
    // In a real application, you would then monitor encoder position or status
    // to know when the movement is complete.
  } else {
    Serial.println("M1SPEEDDIST command failed.");
  ...
  if (millis() - lastCommandTime > 5000) { // Wait 5 seconds before sending the next command
      ...
      if (movingForward) {
          ...
          motor1_speed = 400;
          motor1_distance = 2000; // Move 2000 counts backwards
          motor1_flag = 1 | (1 << 1) | (0 << 3); // bit0=1 (Backward), bit1=1 (Relative), bit3=0 (Immediate)
          Serial.print("Commanding Motor 1: Backward, Relative, ");
          Serial.print(motor1_distance); Serial.print(" counts at ");
          Serial.print(motor1_speed); Serial.println(" counts/sec");
          movingForward = false;
      } else {
          // Command forward relative movement
          motor1_speed = 400;
          motor1_distance = 2000; // Move 2000 counts forwards
          motor1_flag = 0 | (1 << 1) | (0 << 3); // bit0=0 (Forward), bit1=1 (Relative), bit3=0 (Immediate)
          Serial.print("Commanding Motor 1: Forward, Relative, ");
          Serial.print(motor1_distance); Serial.print(" counts at ");
          Serial.print(motor1_speed); Serial.println(" counts/sec");
          movingForward = true;
      }
      // Send the command again with the new parameters
      controller.SpeedDistanceM1(MOTOR_ADDRESS, motor1_speed, motor1_distance, motor1_flag);
      Serial.println("M1SPEEDDIST command sent again.");
  }
