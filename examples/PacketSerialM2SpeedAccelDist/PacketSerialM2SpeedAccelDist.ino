/**
 * Basicmicro Library Example: M2SPEEDACCELDIST (45)
 *
 * Demonstrates commanding Motor 2 to move a specific distance with controlled
...
// Define example acceleration, speed, and distance for Motor 2
uint32_t motor2_accel = 600;     // Acceleration rate in counts/sec/sec
uint32_t motor2_speed = 1200;    // Top speed in counts/sec
uint32_t motor2_distance = 6000; // Distance in encoder counts
...
void setup() {
  ...
  Serial.println("Basicmicro M2SPEEDACCELDIST Example");
...
void loop() {
  // Attempt to send the SpeedAccelDistance command for Motor 2
  // The function expects uint32_t for accel, speed, and distance.
  bool success = controller.SpeedAccelDistanceM2(MOTOR_ADDRESS, motor2_accel, motor2_speed, motor2_distance, motor2_flag);

  if (success) {
    Serial.println("M2SPEEDACCELDIST command successful. Motor should start moving.");
    // In a real application, you would then monitor encoder position or status
    // to know when the movement is complete.
  } else {
    Serial.println("M2SPEEDACCELDIST command failed.");
  ...
  if (millis() - lastCommandTime > 7000) { // Wait 7 seconds before sending the next command
      ...
      if (movingForward) {
          ...
          motor2_accel = 600;
          motor2_speed = 1200;
          motor2_distance = 6000; // Move 6000 counts backwards
          motor2_flag = 1 | (1 << 1) | (0 << 2) | (0 << 3); // bit0=1 (Backward), bit1=1 (Relative), bit2=0 (Accel not limited), bit3=0 (Immediate)
          Serial.print("Commanding Motor 2: Backward, Relative. Accel: "); Serial.print(motor2_accel);
          Serial.print(", Speed: "); Serial.print(motor2_speed);
          Serial.print(", Distance: "); Serial.println(motor2_distance);
          movingForward = false;
      } else {
          // Command forward relative movement
          motor2_accel = 600;
          motor2_speed = 1200;
          motor2_distance = 6000; // Move 6000 counts forwards
          motor2_flag = 0 | (1 << 1) | (0 << 2) | (0 << 3); // bit0=0 (Forward), bit1=1 (Relative), bit2=0 (Accel not limited), bit3=0 (Immediate)
          Serial.print("Commanding Motor 2: Forward, Relative. Accel: "); Serial.print(motor2_accel);
          Serial.print(", Speed: "); Serial.print(motor2_speed);
          Serial.print(", Distance: "); Serial.println(motor2_distance);
          movingForward = true;
      }
      // Send the command again with the new parameters
      controller.SpeedAccelDistanceM2(MOTOR_ADDRESS, motor2_accel, motor2_speed, motor2_distance, motor2_flag);
      Serial.println("M2SPEEDACCELDIST command sent again.");
  }
