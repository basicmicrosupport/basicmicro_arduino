/**
 * Basicmicro Library Example: MIXEDSPEEDPOS (124)
 *
 * Demonstrates commanding both Motor 1 and Motor 2 to move to specific
 * absolute encoder positions at specified maximum speeds in a single command.
 * Each motor uses its own commanded speed and default acceleration/deceleration
 * rates (or potentially PID tuning) to reach its target position.
 *
 * This command requires Position PID control to be enabled and tuned for both motors,
 * and encoders must be configured and connected. The speeds and target positions
 * are 32-bit values (signed concepts).
 *
...
int32_t motor1_speed = 800;      // M1 Maximum speed in counts/sec (signed concept)
int32_t target_pos_1A = 5000;    // M1 First target position (signed 32-bit counts)
int32_t target_pos_1B = -5000;   // M1 Second target position

int32_t motor2_speed = 900;      // M2 Maximum speed in counts/sec (signed concept)
int32_t target_pos_2A = 6000;    // M2 First target position (signed 32-bit counts)
int32_t target_pos_2B = -6000;   // M2 Second target position
...
void setup() {
  ...
  Serial.println("Basicmicro MIXEDSPEEDPOS Example");
  ...
  Serial.print("Attempting initial command to Abs Pos M1: "); Serial.print(target_pos_1A); Serial.print(" at "); Serial.print(motor1_speed);
  Serial.print(", M2: "); Serial.print(target_pos_2A); Serial.print(" at "); Serial.print(motor2_speed);
...
void loop() {
  ...
  if (millis() - lastCommandTime > 8000) { // Wait 8 seconds before sending the next command
      ...
      if (targetIsPosA) {
          current_m1_target_pos = target_pos_1B; // Move to the second target set
          current_m2_target_pos = target_pos_2B;
          targetIsPosA = false;
          Serial.print("Commanding Motors to Abs Pos M1: "); Serial.print(current_m1_target_pos); Serial.print(" at "); Serial.print(motor1_speed);
          Serial.print(", M2: "); Serial.print(current_m2_target_pos); Serial.print(" at "); Serial.println(motor2_speed);
      } else {
          current_m1_target_pos = target_pos_1A; // Move back to the first target set
          current_m2_target_pos = target_pos_2A;
          targetIsPosA = true;
          Serial.print("Commanding Motors to Abs Pos M1: "); Serial.print(current_m1_target_pos); Serial.print(" at "); Serial.print(motor1_speed);
          Serial.print(", M2: "); Serial.print(current_m2_target_pos); Serial.print(" at "); Serial.println(motor2_speed);
      }

      // Attempt to send the MixedSpeedPosition command
      // The speed and position parameters expect uint32_t, so we cast our signed int32_t values.
      bool success = controller.MixedSpeedPosition(MOTOR_ADDRESS,
                                                    (uint32_t)motor1_speed, (uint32_t)current_m1_target_pos,
                                                    (uint32_t)motor2_speed, (uint32_t)current_m2_target_pos,
                                                    buffer_mode);

      if (success) {
        Serial.println("MIXEDSPEEDPOS command successful. Motors should start moving.");
        // In a real application, you might monitor position error or speed to know when they've stopped.
      } else {
        Serial.println("MIXEDSPEEDPOS command failed.");
