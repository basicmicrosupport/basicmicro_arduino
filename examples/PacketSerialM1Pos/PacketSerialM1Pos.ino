/**
 * Basicmicro Library Example: M1POS (119)
 *
 * Demonstrates commanding Motor 1 to move to a specific absolute encoder position
 * using the controller's default speed and acceleration values.
 *
 * This command requires Position PID control to be enabled and tuned for Motor 1,
 * and encoders must be configured and connected. The target position is a
 * 32-bit value (signed concept, sent as uint32_t).
 *
...
// Define example target positions for Motor 1 (signed 32-bit counts)
int32_t target_pos_1 = 5000;   // First target position
int32_t target_pos_2 = -3000;  // Second target position (negative)
...
void setup() {
  ...
  Serial.println("Basicmicro M1POS Example");
  ...
  Serial.print("Attempting to command Motor 1 to initial absolute position: ");
  Serial.println(target_pos_1);
}
...
void loop() {
  ...
  if (millis() - lastCommandTime > 5000) { // Wait 5 seconds before sending the next command
      ...
      if (targetIsPos1) {
          ...
          targetIsPos1 = false;
          Serial.print("Commanding Motor 1 to absolute position: ");
          Serial.println(current_target_pos);
      } else {
      ...
      // Attempt to send the M1Position command
      // The function expects a uint32_t for position, so we cast our signed int32_t.
      bool success = controller.M1Position(MOTOR_ADDRESS, (uint32_t)current_target_pos, buffer_mode);

      if (success) {
        Serial.println("M1POS command successful.");
        // In a real application, you might monitor position error or speed to know when it's stopped.
      } else {
        Serial.println("M1POS command failed.");
