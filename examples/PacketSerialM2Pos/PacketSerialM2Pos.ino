/**
 * Basicmicro Library Example: M2POS (120)
 *
...
// Define example target positions for Motor 2 (signed 32-bit counts)
int32_t target_pos_1 = 6000;   // First target position
int32_t target_pos_2 = -4000;  // Second target position (negative)
...
void setup() {
  ...
  Serial.println("Basicmicro M2POS Example");
  ...
  Serial.print("Attempting to command Motor 2 to initial absolute position: ");
...
void loop() {
  ...
  if (millis() - lastCommandTime > 5000) { // Wait 5 seconds before sending the next command
      ...
      if (targetIsPos1) {
          ...
          targetIsPos1 = false;
          Serial.print("Commanding Motor 2 to absolute position: ");
      ...
      // Attempt to send the M2Position command
      // The function expects a uint32_t for position, so we cast our signed int32_t.
      bool success = controller.M2Position(MOTOR_ADDRESS, (uint32_t)current_target_pos, buffer_mode);

      if (success) {
        Serial.println("M2POS command successful.");
        // In a real application, you might monitor position error or speed to know when it's stopped.
      } else {
        Serial.println("M2POS command failed.");
